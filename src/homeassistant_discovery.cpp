...
static constexpr std::string_view CHARGER_STATE_VALUE_TEMPLATE = "{% set states = "
                                                                 "{ 0: 'Off'"
                                                                 ", 1: 'Low Power'"
                                                                 ", 2: 'Fault'"
                                                                 ", 3: 'Bulk'"
                                                      ...
                                                      const std::unordered_map<std::string, std::unordered_map<std::string, Item>> &all_items)
{
    ...
    std::string deviceIdentifier = vrm_id + "_" + toIdentifier(short_service_name);
                                                        ...
                                                        const std::unordered_map<std::string, std::unordered_map<std::string, Item>> &all_items)
{
    ...
    for (auto & entity : entities) {
        ...
        entity_config.state_topic = "N/" + vrm_id + "/" + short_service_name + dbus_path;
        if (!entity_config.command_topic.empty())
            entity_config.command_topic = "W/" + vrm_id + "/" + short_service_name + entity_config.command_topic;
    }
}
...
void HomeAssistantDiscovery::setEnabledServices(const std::vector<std::string>& services)
{
    ...
    if (!enabled_services.empty()) {
        // Now filter services that are not in the factory functions
        for (auto _ = enabled_services.begin(); _ != enabled_services.end(); ) {
            auto it = _++;
            if (!device_factory_functions.contains(*it)) {
                enabled_services.erase(it);
            }
        }
        flashmq_logf(LOG_DEBUG, "Home Assistant Discovery enabled for %zu specific services", enabled_services.size());
    } else {
        // The given services set was empty (after trimming) so add all known services
        for (const auto &entry : device_factory_functions) {
            enabled_services.insert(std::string(entry.first));
        }
...
{
    return enabled_services.contains(service_type);
}
